...
exit()
"""
# moment estimation of alpha 
def loglikelihood_moment(alpha, y, mu):
    y_max = np.max(y)
    first_term = 0
    for y_i in range(1,y_max+1):
        count = np.count_nonzero(y == y_i)
        ...
        summation = np.sum(np.log(array + 1/alpha))
        first_term += summation * count
    expression =  - np.log(scipy.special.gamma(y+1)) + y*np.log(mu) + y * np.log(alpha) - (y+1/alpha)*np.log(1+alpha*mu)
    l = first_term + np.sum(expression)
    return l
n, p = X.shape
...
while True:
    ...
    l = loglikelihood_moment(alpha, y, mu)
    #diff = l - l_prev 
    ...
    #print('log_mu is in the range of',np.min(updated_log_mu), np.max(updated_log_mu))
    updated_mu = np.exp(updated_log_mu) # shape: (292019, 1)
    print('log_mu is in the range of', np.min(updated_mu), np.max(updated_mu))
    updated_alpha = 1/(n-p) * np.sum((y - mu)**2 /(mu * (1/alpha + mu)))
    alpha = updated_alpha
    count += 1
    print(count, alpha)
    print('-------------------------------')
